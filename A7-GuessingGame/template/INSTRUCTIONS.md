...
How does it know what to ask? The user will teach it! Every time the program gets an answer wrong, it will ask the user to tell it what question would have distinguished the wrong answer from the right answer. It will then add this question to the binary decision tree for use in future games.
...
You will start off with a very simple tree. As the user plays the game, they will help create a more complex tree that is better at guessing. Specifically, when the system guesses wrong, it should ask the user to provide a question that distinguishes the wrong guess from the correct answer. This question will be used to extend the decision tree. 
...
Your `DecisionTree` class should include at least these constructor call signatures:
- `DecisionTree(String data)` for a leaf node
- `DecisionTree(String data, DecisionTree left, DecisionTree right)` for a branch node

In other words, the branch constructor should take `DecisionTree` children specifically, not just `BinaryTree<String>` children. This makes it possible to build and return `DecisionTree` objects consistently throughout the assignment and is the constructor shape the autograder will test for.

You should also write and test a function called `followPath` that accepts a `String` like *YNNYNYY* that it uses to trace a path through the tree from the root.  A *Y* means to follow the left child and a *N* means to follow the right child.  The method should return the `DecisionTree` object located by following that traversal from the root.  Note that you can write this function either iteratively or recursively.
